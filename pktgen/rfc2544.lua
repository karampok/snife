...
require "Pktgen";
DEBUG			= true;
USE_RAMPUP	= true; 	--Rampup traffic flow before starting measurement, less accurate measurement (trial time,drops).
SHOW_STATUS	= true;
--
-- Logfile for testresult
Logfile 		= "RFC2544_throughput_results.txt";
...
duration			= 10000;
confirmDuration	= 60000;			-- If set to 0, Confirm run is skipped.
pauseTime			= 3000;
graceTime			= 5000;				-- Ramp Up grace time
numIterations		= 20;	 			-- Maximum number of divide n conquer iterations
retryCount		= 3;				-- Number of Trial retries if Tx underrun happens
initialRate		= 100;
--
-- Test tolerance settings
loss_tol          = 0.10;
seekPrec			= 0.10;				-- Divide n conquer seek precision 
...
num_ip		= 1; 					-- Number of IPs per interface, max 253. Set DUT IP at .254.
num_udpp		= 1;					-- Number of UDP ports used per IP, max 9999.  
srcip			= "16.1.1.1";
max_srcip		= "16.1.1."..num_ip;
dstip			= "16.2.2.2";
max_dstip		= "16.2.2."..num_ip;
netmask		= "/24";
p0_udpp_src	= 10000;
p0_udpp_dst	= 20000;
p1_udpp_src	= 30000;
p1_udpp_dst	= 40000;
p0_destmac	= "50:00:00:00:00:11";	-- Comment out to test Pktgen loopback
p1_destmac	= "50:00:00:00:00:12";	-- Set to correct DUT MACs
...
function getDuration(pkt_size, set_rate, num_pkts)
...
function checkRate(pkt_size, set_rate, duration, num_pkts)
...
function getTxPkts(set_rate, pkt_size, test_dur)
...
function getPPS(num_pkts, test_dur)
	local test_pps;
...
function getMbpsL2(num_pkts, pkt_size, test_dur)
...
function getMbpsL1(num_pkts, pkt_size, test_dur)
	local test_mbps, test_pps;
...
function runTrial(pkt_size, rate, duration, count, retry)
...
function runThroughputTest(pkt_size)
